...
public class BikeListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Bike> list = new AdminService().selectBikeList(cPage,numPerPage,shopCode);
		int totalContent=new AdminService().selectShopTotalCount();
...
//		이전
		if(pageNo ==1) {
			...
			pageBar += "<ul class='"+"pagination"+"'><li><a href='"+request.getContextPath()+"/bikeList?cPage="+(pageNo-1)+"&numPerPage="+numPerPage+"' ><span>&laquo;</span></a></li></ul>";
		}
		while(!(pageNo > pageEnd|| pageNo>totalPage)) {
			if(cPage == pageNo) { 
				...
				pageBar += "<ul class='"+"pagination"+"'><li><a href='"+request.getContextPath()+"/bikeList?cPage="+pageNo+"&numPerPage="+numPerPage+"' >"+pageNo+"</a></li></ul>";
				//             
			}
			pageNo++;
		}
		...
		if(pageNo > totalPage) {
			...
			pageBar += "<ul class='"+"pagination"+"'><li><a href='"+request.getContextPath()+"/bikeList?cPage="+pageNo+"&numPerPage="+numPerPage+"' ><span>&raquo;</span></a></li></ul>";
		}
